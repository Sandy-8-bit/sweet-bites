...
export const Navbar = () => {
  ...
  let Links = [
    { name: 'Home', link: '#', icon: <GoHomeFill className='w-5 h-5 lg:hidden' /> },
    { name: 'About', link: '#about', icon: <FaCircleInfo className='lg:hidden' /> },
    { name: 'Services', link: '#services', icon: <RiCustomerService2Fill className='w-5 h-5 lg:hidden' /> },
    { name: 'Menu', link: '#menu', icon: <BiSolidFoodMenu className='w-5 h-5 lg:hidden' /> },
    { name: 'Contact', link: '#contact', icon: <MdContactPhone className='w-5 h-5 lg:hidden' /> }
  ]
  ...
  return (
    <nav className='shadow-md w-[90%] z-50 fixed top-3 right-1/2 left-1/2 -translate-x-1/2 py-3 px-7 bg-light/70 backdrop-blur-md rounded-full text-dark'>
      <div ref={menuRef} className='flex items-center justify-between'>
        <div>
          <a href='/'>
            <img className='w-8 md:w-10' src='logo.png' alt='Sweet Bite' />
          </a>
        </div>
